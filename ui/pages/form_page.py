...
class FormPage(AbstractPage):
    ...
    def _render_realty_inputs(self):
        ...
        d["realty_type"] = rt.selectbox("Тип недвижимости",
                                        options=REALTY_TYPE_OPTIONS)
        d["material"] = rt.selectbox("Материал дома", options=MATERIAL_OPTIONS)
        if d.get("realty_type") == "Квартира":
            col1, col2 = rt.columns(2)
            d["levels"] = int(
                col1.number_input("Количество этажей",
                                  step=1,
                                  min_value=1,
                                  max_value=999,
                                  value=5))
            d["level"] = int(
                col2.number_input("Этаж",
                                  step=1,
                                  min_value=1,
                                  max_value=d.get("levels")))
            d["is_studio"] = bool(rt.checkbox("Студия"))
        else:
            d["levels"] = int(
                rt.number_input("Количество этажей",
                                step=1,
                                min_value=1,
                                max_value=999,
                                value=5))

        d["rooms"] = (0 if d.get("is_studio") else int(
            rt.number_input(
                "Количество комнат", step=1, min_value=1, max_value=100)))
        col1, col2 = rt.columns(2)
        d["area"] = col1.number_input("Площадь (кв. м)",
                                      step=0.1,
                                      min_value=0.0,
                                      max_value=FLOAT_MAX)
        d["kitchen_area"] = col2.number_input("Площадь кухни (кв. м)",
                                              step=0.1,
                                              min_value=0.0,
                                              max_value=d.get("area"))
        col1, col2 = rt.columns(2)
        d["geo_lat"] = col1.number_input("Широта",
                                         value=0.0,
                                         min_value=-90.0,
                                         max_value=90.0)
        d["geo_lon"] = col2.number_input("Долгота",
                                         value=0.0,
                                         min_value=-180.0,
                                         max_value=180.0)
    ...
    def _render_vehicle_inputs(self):
        ...
        d["year"] = col2.number_input("Год выпуска",
                                      step=1,
                                      min_value=1970,
                                      max_value=2021)
        if d.get("brand") != "Другое":
            col, *cols = rt.columns(3)
            d["model"] = col.selectbox("Модель",
                                       options=MODEL_OPTIONS_BY_BRAND.get(
                                           d.get("brand"), []))
        else:
            cols = rt.columns(2)
        d["body_type"] = cols[0].selectbox("Кузов", options=BODY_TYPE_OPTIONS)
        d["color"] = cols[1].selectbox("Цвет", options=COLOR_OPTIONS)
        col1, col2 = rt.columns([1, 2])
        d["pts"] = col1.selectbox("ПТС", options=PTS_TYPE_OPTIONS)

        d["complectation"] = col2.text_input(
            "Комплектация (Значения через запятую)")

        cols = rt.columns(3)
        d["fuel_type"] = cols[0].selectbox("Топливо",
        ...
        d["engine_power"] = cols[2].number_input("Мощность двигателя",
                                                 step=1,
                                                 min_value=0)

        cols = rt.columns(2)
        d["drive"] = cols[0].selectbox("Привод", options=DRIVE_OPTIONS)
        d["transmission"] = cols[1].selectbox("Трансмиссия",
                                              options=TRANSMISSION_OPTIONS)

        cols = rt.columns(3)
        d["wheel"] = cols[0].selectbox("Руль", options=WHEEL_OPTIONS)
        d["mileage"] = cols[1].number_input("Пробег",
                                            step=1,
                                            min_value=0,
                                            max_value=INT_MAX)
        d["owners_count"] = cols[2].number_input("Количество владельцев",
                                                 step=1,
                                                 min_value=1)
    ...
    def render(self):
        ...
        d["property_type"] = rt.radio("Тип имущества",
