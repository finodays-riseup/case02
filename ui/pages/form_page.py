...
class FormPage(AbstractPage):
    ...
    def _render_realty_inputs(self):
        ...
        d["building_type"] = rt.selectbox("Материал дома", options=MATERIAL_OPTIONS)
        ...
        d["level"] = int(
            col2.number_input("Этаж", step=1, min_value=1, max_value=d.get("levels"))
        )
        ...
        d["area"] = col1.number_input(
            "Площадь (кв. м)", value=40.0, step=0.1, min_value=0.0, max_value=FLOAT_MAX
        )
        ...
        d["geo_lat"] = col1.number_input(
            "Широта", value=55.7558, min_value=-90.0, max_value=90.0
        )
        d["geo_lon"] = col2.number_input(
            "Долгота", value=37.6173, min_value=-180.0, max_value=180.0
        )
    ...
    def _render_vehicle_inputs(self):
        ...
        d["complectation"] = col2.text_input("Комплектация")
